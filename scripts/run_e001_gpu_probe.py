...
def run_probe_gpu(view_col, config_path, data_dir, artifact_dir):
    print(f"=== E001-C GPU PROBE: {view_col} ===")
    ...
    # Use cuml TfidfVectorizer if possible, else sklearn
    try:
        from cuml.feature_extraction.text import TfidfVectorizer as cuml_TfidfVectorizer
        TFIDF = cuml_TfidfVectorizer
        print("Using cuML TfidfVectorizer")
    except ImportError:
        TFIDF = TfidfVectorizer
        print("cuML not found. Falling back to sklearn TfidfVectorizer on CPU.")
    ...
    def retrieve(source_df, source_name):
        ...
        for country in countries:
            ...
            # Convert series to appropriate format for cuML if needed. cuML takes Series of strings.
            try:
                X_target = vec.fit_transform(s_sX[view_col])
            except Exception as e:
                # If cuML fails, fallback to CPU
                print(f"cuML fit failed: {e}. Falling back to CPU vectorization.")
                vec = TfidfVectorizer(**vectorizer_kwargs)
                X_target = vec.fit_transform(s_sX[view_col])
                
            t_fit = time.time() - t_fit_start
            total_fit_time += t_fit
            
            t_transform_start = time.time()
            X_query = vec.transform(s_s1[view_col])
            t_transform = time.time() - t_transform_start
            total_transform_time += t_transform
            
            # If cuML returned CuPy array, we convert to scipy for API consistency if needed, but sp_matmul_topn_cupy handles it internally.
            # To be safe, we bring to CPU scipy sparse to feed into our cupy matmul wrapper because it explicitly wraps it.
            ...
            t_topn_start = time.time()
            # Perform GPU sparse dot Top-K
            top_sparse = sp_matmul_topn_cupy(
                X_query, X_target.T, top_k=config['top_k'], batch_size=config.get('batch_size', 1000)
            )
            t_topn = time.time() - t_topn_start
            total_topn_time += t_topn
            
            # Extract indices from sparse result
            ...
            for i, s1_id in enumerate(s_s1_ids):
                ...
                # the similarities are in top_sparse.data[row_start:row_end]
        ...
        print(f"Total Queries: {total_queries}, Total Targets: {total_docs}")
        print(f"Total Fit Time: {total_fit_time:.2f}s")
        print(f"Total Transform Time: {total_transform_time:.2f}s")
        print(f"Total Top-N GPU Mul Time: {total_topn_time:.2f}s")
        print(f"Total Source Time: {t_total:.2f}s (Throughput: {qps:.1f} Q/sec)")
        
        projected_time = (total_s1 / qps) if qps > 0 else 0
        print(f"Projected Full {source_name} Time for {total_s1} queries: {projected_time/3600:.2f} hours")
        
        return retrieved_dict
        
    s2_retrieved = retrieve(s2, "S2")
    print(f"Peak Memory after S2: {memory_usage():.1f} MB")
    
    s3_retrieved = retrieve(s3, "S3")
    print(f"Peak Memory after S3: {memory_usage():.1f} MB")
    ...
    print(f"Total E001-C Probe Time: {time.time() - start_time:.1f}s")
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, default='configs/e001_gpu.yaml')
    parser.add_argument('--data-dir', type=str, default='data')
    parser.add_argument('--out-dir', type=str, default='artifacts/retrieval/E001-C')
    args = parser.parse_args()
    
    # Kaggle env vars override defaults
