import os
import json
import time
import argparse
import pandas as pd
import numpy as np
import psutil
import yaml
from pathlib import Path
from sklearn.feature_extraction.text import TfidfVectorizer

from src.business_entity_resolution.retrieval.sparse_topk import sparse_top_k

def memory_usage():
    process = psutil.Process(os.getpid())
    return process.memory_info().rss / (1024 * 1024)

def sample_s1(folds_df, n_samples=50000, seed=42):
    sampled = folds_df.groupby('match_bucket', group_keys=False).apply(
        lambda x: x.sample(n=min(len(x), int(n_samples * len(x)/len(folds_df))), random_state=seed),
        include_groups=False
    )
    # The output might drop match_bucket if include_groups=False is passed but the index behavior might differ
    # For safety in older pandas versions, using old way and suppressing warning or returning source1_entity_id
    pass

def safe_sample_s1(folds_df, n_samples=50000, seed=42):
    # Sample without apply warning
    sampled = folds_df.groupby('match_bucket').sample(
        frac=n_samples/len(folds_df), random_state=seed
    )
    return sampled['source1_entity_id'].values

def evaluate_retrieval(retrieved_pairs, gt_dict, s1_sample, k_vals):
    results = {}
    for k in k_vals:
        gt_found = 0
        total_gt = 0
        full_cov_count = 0
        s1_with_gt = 0
        cands_generated = 0
        
        for s1_id in s1_sample:
            gt_matches = gt_dict.get(s1_id, set())
            cands = retrieved_pairs.get(s1_id, [])[:k]
            
            cands_generated += len(cands)
            
            if len(gt_matches) > 0:
                total_gt += len(gt_matches)
                s1_with_gt += 1
                
                found = len(gt_matches.intersection(set(cands)))
                gt_found += found
                if found == len(gt_matches):
                    full_cov_count += 1
                    
        pair_recall = gt_found / total_gt if total_gt > 0 else 0
        full_cov = full_cov_count / s1_with_gt if s1_with_gt > 0 else 0
        mean_cands = cands_generated / len(s1_sample)
        
        results[k] = {
            "pair_recall": pair_recall,
            "full_coverage": full_cov,
            "mean_candidates": mean_cands
        }
    return results

def run_probe_b(view_col):
    print(f"=== E001-B-PROBE: {view_col} ===")
    start_time = time.time()
    
    with open("configs/e001.yaml", "r") as f:
        config = yaml.safe_load(f)['retrieval']
        
    print(f"Loaded config: {config}")
    
    # 1. Load Data
    s1 = pd.read_parquet("data/processed/v001/train_source1.parquet")
    s2 = pd.read_parquet("data/processed/v001/train_source2.parquet")
    s3 = pd.read_parquet("data/processed/v001/train_source3.parquet")
    gt = pd.read_csv("data/raw/train/train_ground_truth.tsv", sep="\t", dtype=str).fillna("")
    folds = pd.read_parquet("artifacts/folds/folds_v1.parquet")
    
    total_s1 = len(s1)
    
    # 2. Sample S1
    s1_sample_ids = safe_sample_s1(folds, n_samples=50000)
    s1_sample = s1[s1['entity_id'].isin(s1_sample_ids)].copy()
    
    print(f"Total S1 entities (for projection): {total_s1}")
    print(f"Sampled S1: {len(s1_sample)} entities")
    
    # 3. Ground Truth
    gt_s2 = {}
    gt_s3 = {}
    s1_sample_ids_set = set(s1_sample_ids)
    for row in gt.itertuples():
        s = row.source1_entity_id
        if s not in s1_sample_ids_set:
            continue
        matches = row.matched_entity_ids
        if matches:
            for match in matches.split(','):
                if match.startswith('S2'):
                    gt_s2.setdefault(s, set()).add(match)
                elif match.startswith('S3'):
                    gt_s3.setdefault(s, set()).add(match)
                    
    s1_sample['is_empty'] = s1_sample[view_col].isna() | (s1_sample[view_col] == "")
    valid_s1 = s1_sample[~s1_sample['is_empty']]
    
    vectorizer_kwargs = {
        'analyzer': config['analyzer'],
        'ngram_range': (config['ngram_min'], config['ngram_max']),
        'sublinear_tf': True,
        'dtype': np.float32 if config['dtype'] == 'float32' else np.float64,
        'min_df': config['min_df'],
        'max_df': config['max_df'],
        'max_features': config['max_features']
    }
    
    def retrieve(source_df, source_name):
        print(f"\n--- Processing {source_name} ---")
        retrieved_dict = {}
        countries = valid_s1['country'].unique()
        
        t0 = time.time()
        
        total_queries = 0
        total_docs = 0
        total_fit_time = 0
        total_transform_time = 0
        total_topn_time = 0
        
        for country in countries:
            s_s1 = valid_s1[valid_s1['country'] == country]
            s_sX = source_df[source_df['country'] == country]
            s_sX = s_sX[~s_sX[view_col].isna() & (s_sX[view_col] != "")]
            
            if len(s_s1) == 0 or len(s_sX) == 0:
                continue
                
            total_queries += len(s_s1)
            total_docs += len(s_sX)
                
            vec = TfidfVectorizer(**vectorizer_kwargs)
            
            t_fit_start = time.time()
            X_target = vec.fit_transform(s_sX[view_col])
            t_fit = time.time() - t_fit_start
            total_fit_time += t_fit
            
            t_transform_start = time.time()
            X_query = vec.transform(s_s1[view_col])
            t_transform = time.time() - t_transform_start
            total_transform_time += t_transform
            
            if country == 'US' or country == 'India':
                print(f"[{country}] Target Matrix shape: {X_target.shape}, NNZ: {X_target.nnz}, Vocab: {len(vec.vocabulary_)}")
                print(f"[{country}] Query Matrix shape: {X_query.shape}, NNZ: {X_query.nnz}")
                
            t_topn_start = time.time()
            top_indices, top_scores = sparse_top_k(
                X_query, X_target.T, k=config['top_k'], chunk_size=1000
            )
            t_topn = time.time() - t_topn_start
            total_topn_time += t_topn
            
            s_sX_ids = s_sX['entity_id'].values
            s_s1_ids = s_s1['entity_id'].values
            
            for i, s1_id in enumerate(s_s1_ids):
                valid_idx = top_indices[i] >= 0
                match_ids = s_sX_ids[top_indices[i][valid_idx]]
                retrieved_dict[s1_id] = list(match_ids)
                
        t_total = time.time() - t0
        qps = total_queries / t_total if t_total > 0 else 0
        
        print(f"{source_name} Summary:")
        print(f"Total Queries: {total_queries}, Total Targets: {total_docs}")
        print(f"Total Fit Time: {total_fit_time:.2f}s")
        print(f"Total Transform Time: {total_transform_time:.2f}s")
        print(f"Total Top-N Mul Time: {total_topn_time:.2f}s")
        print(f"Total Source Time: {t_total:.2f}s (Throughput: {qps:.1f} Q/sec)")
        
        # Projection
        projected_time = (total_s1 / qps) if qps > 0 else 0
        print(f"Projected Full {source_name} Time for {total_s1} queries: {projected_time/3600:.2f} hours")
        
        return retrieved_dict
        
    s2_retrieved = retrieve(s2, "S2")
    print(f"Peak Memory after S2: {memory_usage():.1f} MB")
    
    s3_retrieved = retrieve(s3, "S3")
    print(f"Peak Memory after S3: {memory_usage():.1f} MB")
    
    # Evaluate
    k_vals = [1, 3, 5, 10, 20]
    print("\n--- S2 Results ---")
    s2_eval = evaluate_retrieval(s2_retrieved, gt_s2, set(s1_sample_ids), k_vals)
    for k in k_vals:
        print(f"K={k}: PairRecall={s2_eval[k]['pair_recall']:.4f}, FullCov={s2_eval[k]['full_coverage']:.4f}, Cands/S1={s2_eval[k]['mean_candidates']:.2f}")
        
    print("\n--- S3 Results ---")
    s3_eval = evaluate_retrieval(s3_retrieved, gt_s3, set(s1_sample_ids), k_vals)
    for k in k_vals:
        print(f"K={k}: PairRecall={s3_eval[k]['pair_recall']:.4f}, FullCov={s3_eval[k]['full_coverage']:.4f}, Cands/S1={s3_eval[k]['mean_candidates']:.2f}")

    print(f"\nFinal Memory Usage: {memory_usage():.1f} MB")
    print(f"Total E001-B Probe Time: {time.time() - start_time:.1f}s")
    
if __name__ == "__main__":
    # We test only name_norm_clean to not do double work before acceptance
    run_probe_b("name_norm_clean")
