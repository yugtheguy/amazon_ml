...
notebook = {
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "## 5. Engineering Smoke Check + Pre-Flight Audit\n",
    "\n",
    "Runs:\n",
    "1. 1K smoke retrieval (with checkpoint/resume)\n",
    "2. Full pre-full-run audit on the smoke artifacts **before** cleanup\n",
    "3. Cleans up smoke artifacts only after both pass"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "    subprocess.run(\n",
    "        [sys.executable, \"-u\", \"scripts/build_folds.py\"],\n",
    "        check=True, env={**os.environ, \"PYTHONPATH\": \".\"}\n",
    "    )\n",
    "\n",
    "SMOKE_C001_DIR = \"/kaggle/working/artifacts/candidate_pool/C001\"\n",
    "SMOKE_POOL_DIR = SMOKE_C001_DIR + \"/candidate_pool_v1\"\n",
    "env = os.environ.copy()\n",
    "env[\"PYTHONPATH\"] = \".\"\n",
    "\n",
    ...
    "]\n",
    "try:\n",
    "    subprocess.run(smoke_cmd, env=env, check=True)\n",
    "except subprocess.CalledProcessError as e:\n",
    "    print(\"SMOKE FAILED\")\n",
    "    raise e\n",
    "\n",
    "# Audit runs while smoke artifacts are still present\n",
    "print(\"\\n=== Running pre-full-run audit on smoke artifacts ===\")\n",
    "audit_cmd = [\n",
    "    sys.executable, \"-u\", \"scripts/audit_c001_smoke.py\",\n",
    "    \"--candidate-pool-dir\", SMOKE_POOL_DIR,\n",
    "    \"--fold-manifest\", FOLD_MANIFEST,\n",
    "    \"--processed-dir\", PROCESSED_DATA_ROOT,\n",
    "    \"--data-dir\", \"data\"\n",
    "]\n",
    "try:\n",
    "    subprocess.run(audit_cmd, env=env, check=True)\n",
    "    print(\"\\n=== AUDIT PASSED ===\")\n",
    "except subprocess.CalledProcessError as e:\n",
    "    print(\"AUDIT FAILED — review output above before proceeding to full run.\")\n",
    "    raise e\n",
    "\n",
    "# Clean up smoke artifacts only after both smoke and audit pass\n",
    "shutil.rmtree(SMOKE_C001_DIR)\n",
