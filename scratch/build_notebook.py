import json
import os

notebook = {
 "cells": [
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "# R001: Efficient Hybrid Candidate Generator V1\n",
    "\n",
    "## 00 — Experiment contract\n",
    "This notebook executes the R001 candidate generation pipeline. It uses cuML TF-IDF for text fields and exact/rare/numeric matching. DO NOT RUN 50K until 1K smoke passes."
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 01 — GPU environment"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "import sys\n",
    "import os\n",
    "import subprocess\n",
    "\n",
    "print(f\"Python Version: {sys.version}\")\n",
    "print(f\"CWD: {os.getcwd()}\")\n",
    "!nvidia-smi\n",
    "\n",
    "try:\n",
    "    import cuml\n",
    "    import cupy as cp\n",
    "    print(\"cuML/CuPy is available. GPU_BACKEND_ACTIVE = TRUE\")\n",
    "except ImportError:\n",
    "    print(\"WARNING: cuML/CuPy not found.\")\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 02 — GitHub clone/reuse"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "import subprocess\n",
    "REPO_URL = \"https://github.com/yugtheguy/amazon_ml.git\"\n",
    "REPO_DIR = \"/kaggle/working/amazon_ml\"\n",
    "if not os.path.exists(REPO_DIR):\n",
    "    print(f\"Cloning {REPO_URL} ...\")\n",
    "    subprocess.run([\"git\", \"clone\", REPO_URL, REPO_DIR], check=True)\n",
    "else:\n",
    "    print(\"Repo exists, pulling latest...\")\n",
    "    subprocess.run([\"git\", \"-C\", REPO_DIR, \"pull\"], check=True)\n",
    "os.chdir(REPO_DIR)\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 03 — Commit/version"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "!git status --short\n",
    "!git rev-parse HEAD\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 04 — Dataset discovery"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "KAGGLE_DATA_ROOT = None\n",
    "for root, dirs, files in os.walk(\"/kaggle/input\"):\n",
    "    if \"train_source1.tsv\" in files and \"train\" in root:\n",
    "        KAGGLE_DATA_ROOT = os.path.dirname(root)\n",
    "        break\n",
    "\n",
    "if not KAGGLE_DATA_ROOT:\n",
    "    KAGGLE_DATA_ROOT = os.path.abspath(\"data/raw\")\n",
    "print(f\"RAW_DATA_ROOT = {KAGGLE_DATA_ROOT}\")\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 05 — Processed data prepare/reuse"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "PROCESSED_DATA_ROOT = \"/kaggle/working/amazon_ml_data/processed/v001\"\n",
    "if not os.path.exists(PROCESSED_DATA_ROOT + \"/train_source1.parquet\"):\n",
    "    print(\"Processed data not found. Symlinking raw data and building...\")\n",
    "    os.makedirs(\"data/raw\", exist_ok=True)\n",
    "    if not os.path.exists(\"data/raw/train\") and os.path.exists(os.path.join(KAGGLE_DATA_ROOT, \"train\")):\n",
    "        os.symlink(os.path.join(KAGGLE_DATA_ROOT, \"train\"), \"data/raw/train\")\n",
    "    if not os.path.exists(\"data/raw/test\") and os.path.exists(os.path.join(KAGGLE_DATA_ROOT, \"test\")):\n",
    "        os.symlink(os.path.join(KAGGLE_DATA_ROOT, \"test\"), \"data/raw/test\")\n",
    "    \n",
    "    os.makedirs(\"/kaggle/working/amazon_ml_data\", exist_ok=True)\n",
    "    os.environ[\"KAGGLE_DATA_ROOT\"] = \"/kaggle/working/amazon_ml_data\"\n",
    "    !PYTHONPATH=. KAGGLE_DATA_ROOT=/kaggle/working/amazon_ml_data python scripts/build_processed_data.py\n",
    "    !PYTHONPATH=. KAGGLE_DATA_ROOT=/kaggle/working/amazon_ml_data python scripts/build_folds.py\n",
    "else:\n",
    "    print(\"Processed data found. Reusing.\")\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 06 — Tests"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "!pip install -r requirements.txt -q\n",
    "!PYTHONPATH=. ALLOW_CPU_TFIDF=1 python -m pytest tests/ -q\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 07 — Config"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "import yaml\n",
    "import json\n",
    "with open(\"configs/candidate_pool_v1.yaml\", \"r\") as f:\n",
    "    config = yaml.safe_load(f)\n",
    "print(json.dumps(config, indent=2))\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 08 — 1K smoke"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "ARTIFACT_ROOT = \"/kaggle/working/artifacts/candidate_pool/R001\"\n",
    "!PYTHONPATH=. python -u scripts/run_r001_candidate_pool.py --smoke-size 1000 --probe-size 0 --data-dir /kaggle/working/amazon_ml_data --out-dir /kaggle/working/artifacts/candidate_pool/R001/smoke\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 09 — Smoke metrics"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "if os.path.exists(\"/kaggle/working/artifacts/candidate_pool/R001/smoke/retrieval_metrics.json\"):\n",
    "    with open(\"/kaggle/working/artifacts/candidate_pool/R001/smoke/retrieval_metrics.json\") as f:\n",
    "        print(json.dumps(json.load(f), indent=2))\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 10 — 50K run\n",
    "**RUN ONLY AFTER 1K SMOKE PASSES**"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "!PYTHONPATH=. python -u scripts/run_r001_candidate_pool.py --smoke-size 0 --probe-size 50000 --data-dir /kaggle/working/amazon_ml_data --out-dir /kaggle/working/artifacts/candidate_pool/R001/probe_50k\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 11 — Candidate frontier"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "import pandas as pd\n",
    "f = \"/kaggle/working/artifacts/candidate_pool/R001/probe_50k/candidate_count_vs_recall.csv\"\n",
    "if os.path.exists(f):\n",
    "    display(pd.read_csv(f))\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 12 — Channel contributions"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "f2 = \"/kaggle/working/artifacts/candidate_pool/R001/probe_50k/channel_contribution.csv\"\n",
    "if os.path.exists(f2):\n",
    "    display(pd.read_csv(f2))\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 13 — Miss analysis"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "f3 = \"/kaggle/working/artifacts/candidate_pool/R001/probe_50k/missed_gt_pairs.parquet\"\n",
    "if os.path.exists(f3):\n",
    "    missed = pd.read_parquet(f3)\n",
    "    print(f\"Total Missed GT pairs: {len(missed)}\")\n",
    "    display(missed.head())\n"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "## 14 — Artifact packaging"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": None,
   "metadata": {},
   "outputs": [],
   "source": [
    "import shutil\n",
    "shutil.make_archive(\"/kaggle/working/R001_artifacts\", 'zip', \"/kaggle/working/artifacts/candidate_pool/R001\")\n",
    "print(\"Artifacts packaged to /kaggle/working/R001_artifacts.zip\")\n"
   ]
  }
 ],
 "metadata": {
  "kernelspec": {
   "display_name": "Python 3",
   "language": "python",
   "name": "python3"
  }
 },
 "nbformat": 4,
 "nbformat_minor": 5
}

with open("kaggle/01_candidate_pool_v1.ipynb", "w") as f:
    json.dump(notebook, f, indent=1)
